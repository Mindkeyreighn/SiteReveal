...
function contactLinks(facts, light = false, includeMaps = true) {
  ...
  return [
    ...
    includeMaps && facts.googleMapsUrl ? `<a class="${cls} button-outline" href="${href(facts.googleMapsUrl)}" target="_blank" rel="noopener">View on Google Maps</a>` : ''
  ].filter(Boolean).join('');
}
...
function heroCopy(facts, spec) {
  return `<div class="hero-copy"><div class="kicker">${[facts.category, facts.locationLabel].filter(Boolean).map(esc).join(' · ')}</div><h1>${esc(spec.headline)}</h1><p class="lede">${esc(spec.description)}</p><div class="actions">${contactLinks(facts, false, false)}<a class="button button-outline" href="#services">Explore details</a></div></div>`;
}

function contact(facts, spec) {
  return `<section class="contact-section" id="contact" data-layout="contact"><div class="contact-panel contact-message"><div class="kicker">Contact ${esc(facts.businessName)}</div><h2>${esc(spec.contactTitle)}</h2><p>${esc(spec.contactIntro)}</p></div><div class="contact-panel contact-details"><div class="kicker">${esc(facts.locationLabel || 'Direct contact')}</div><h2>Make the first conversation count.</h2><div class="contact-actions">${contactLinks(facts, true)}</div></div></section>`;
}

function scopeBuilder(spec) {
  return `<form class="scope-builder" data-signature-experience="interactive-checklist"><fieldset><legend>Build a project note</legend>${spec.signatureModuleItems.map((item, index) => `<label><input type="checkbox" name="scope-${index + 1}"><span><b>0${index + 1}</b>${esc(item)}</span></label>`).join('')}</fieldset><label class="notes-label">Project notes<textarea aria-label="Project notes" placeholder="Rooms, surfaces, conditions, timing, colors, and questions"></textarea></label><p>Nothing entered here is transmitted. Use it as a checklist for your call.</p></form>`;
}
...
function fieldEditorial(facts, spec, artwork, heroImage) {
  return `<main id="top" data-family-renderer="field-editorial">
    ...
    <section class="signature signature-board" id="guide" data-layout="signature-board"><div class="wrap signature-grid"><div><div class="kicker">Interactive project planner</div><h2>${esc(spec.signatureModuleTitle)}</h2><p>${esc(spec.signatureModuleIntro)}</p></div>${scopeBuilder(spec)}</div></section>
...
function heritageSpecialist(facts, spec, artwork, heroImage) {
  return `<main id="top" data-family-renderer="heritage-specialist">
    ...
    <section class="signature heritage-guide" id="guide" data-layout="two-panel-guide" data-signature-experience="specialist-field-guide"><div class="wrap"><div class="guide-title"><div class="kicker">Specialist field guide</div><h2>${esc(spec.signatureModuleTitle)}</h2><p>${esc(spec.signatureModuleIntro)}</p></div><ol>${signatureItems(spec)}</ol></div></section>
...
function premiumAppointment(facts, spec, artwork, heroImage) {
  return `<main id="top" data-family-renderer="premium-appointment">
    ...
    <section class="signature premium-lookbook" id="guide" data-layout="lookbook" data-signature-experience="personal-lookbook"><div class="wrap"><div class="guide-title"><div class="kicker">Personal style brief</div><h2>${esc(spec.signatureModuleTitle)}</h2><p>${esc(spec.signatureModuleIntro)}</p></div><ol>${signatureItems(spec)}</ol></div></section>
...
function warmNeighborhood(facts, spec, artwork, heroImage) {
  return `<main id="top" data-family-renderer="warm-neighborhood">
    ...
    <section class="signature warm-guide" id="guide" data-layout="postcard-guide" data-signature-experience="neighborhood-postcard"><div class="wrap signature-grid"><div><div class="kicker">Bring the right details</div><h2>${esc(spec.signatureModuleTitle)}</h2><p>${esc(spec.signatureModuleIntro)}</p></div><ol>${signatureItems(spec)}</ol></div></section>
...
function technicalAuthority(facts, spec, artwork, heroImage) {
  return `<main id="top" data-family-renderer="technical-authority">
    ...
    <section class="signature technical-console" id="guide" data-layout="diagnostic-console" data-signature-experience="diagnostic-console"><div class="wrap signature-grid"><div><div class="kicker">Diagnostic starting point</div><h2>${esc(spec.signatureModuleTitle)}</h2><p>${esc(spec.signatureModuleIntro)}</p></div><ol>${signatureItems(spec)}</ol></div></section>
...
function landscapeLocal(facts, spec, artwork, heroImage) {
  return `<main id="top" data-family-renderer="landscape-local">
    ...
    <section class="signature property-map" id="guide" data-layout="property-map" data-signature-experience="property-map"><div class="wrap signature-grid"><div><div class="kicker">Walk the whole property</div><h2>${esc(spec.signatureModuleTitle)}</h2><p>${esc(spec.signatureModuleIntro)}</p></div><ol>${signatureItems(spec)}</ol></div></section>
...
function styles(spec) {
  return `<style>
    :root{--ink:#0a1511;--paper:${esc(spec.secondaryColor)};--white:#fff;--primary:${esc(spec.primaryColor)};--secondary:${esc(spec.secondaryColor)};--accent:${esc(spec.accentColor)};--surface:${esc(spec.surfaceColor)};--line:#d9ddd7;--muted:#53605b;--radius:28px}
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:var(--paper);color:var(--ink);font:16px/1.55 Arial,sans-serif}a{color:inherit}.wrap{width:min(1180px,calc(100% - 40px));margin:auto}.preview{padding:10px 18px;text-align:center;background:#0a1511;color:#fff;font-size:12px;font-weight:900;letter-spacing:.09em}.site-header{min-height:92px;display:flex;align-items:center;gap:28px}.brand{display:flex;align-items:center;gap:12px;text-decoration:none;font-weight:900;font-size:18px}.brand-mark{width:50px;height:50px;border-radius:14px;display:grid;place-items:center;background:var(--primary);color:#fff}.site-header nav{margin-left:auto;display:flex;gap:22px}.header-call{padding:12px 18px;border-radius:999px;background:var(--ink);color:#fff;text-decoration:none;font-weight:800}
    ...
    .signature{padding:100px 0;background:var(--ink);color:#fff}.signature-grid,.heritage-guide .wrap,.premium-lookbook .wrap{display:grid;grid-template-columns:.85fr 1.15fr;gap:70px;align-items:start}.signature .kicker{color:var(--accent)}.signature p{color:#c8d2ce}.signature ol{list-style:none;margin:0;padding:0;display:grid;gap:12px}.signature li{display:grid;grid-template-columns:46px 1fr;align-items:center;padding:18px;border:1px solid #ffffff24;border-radius:15px;background:#ffffff08}.signature li b{color:var(--accent)}.scope-builder{background:var(--surface);color:var(--ink);padding:34px;border-radius:48px 48px 12px 48px}.scope-builder fieldset{border:0;padding:0;margin:0}.scope-builder legend{font-size:13px;font-weight:900;letter-spacing:.14em;text-transform:uppercase;margin-bottom:14px}.scope-builder label{display:block}.scope-builder fieldset label{border-top:1px solid #0002;padding:13px 0;cursor:pointer}.scope-builder input{position:absolute;opacity:0}.scope-builder fieldset span{display:grid;grid-template-columns:46px 1fr;gap:10px;align-items:center}.scope-builder fieldset span:before{content:'✓';grid-column:1;width:26px;height:26px;border:1px solid var(--ink);border-radius:50%;display:grid;place-items:center;color:transparent}.scope-builder fieldset b{display:none}.scope-builder input:checked+span:before{background:var(--accent);border-color:var(--accent);color:var(--ink)}.notes-label{font-size:11px;font-weight:900;letter-spacing:.12em;text-transform:uppercase;margin-top:18px}.scope-builder textarea{width:100%;min-height:78px;margin-top:8px;border:0;border-bottom:1px solid var(--ink);background:transparent;font:16px Arial,sans-serif;resize:vertical}.scope-builder p{color:var(--muted);font-size:12px}.trust-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.trust-item{padding:30px;border:1px solid var(--line);border-radius:34px 34px 8px 34px;background:#fff}.trust-item:nth-child(2){background:var(--surface)}.trust-item:nth-child(3){background:var(--accent)}.trust-item>b{color:var(--primary)}.process-grid,.intro-grid{display:grid;grid-template-columns:.85fr 1.15fr;gap:80px}.process-list{display:grid;gap:12px}.process-list.horizontal{grid-template-columns:repeat(4,1fr)}.process-step{display:grid;grid-template-columns:48px 1fr;gap:10px;padding:22px;border-top:2px solid var(--primary)}.process-list.horizontal .process-step{display:block}.process-step>b{color:var(--primary)}.contact-section{display:grid;grid-template-columns:1fr 1fr;color:#fff}.contact-panel{padding:90px max(36px,7vw)}.contact-message{background:var(--accent);color:var(--ink)}.contact-details{background:var(--ink)}.contact-panel p{font-size:18px}.contact-panel h2{font-family:Georgia,serif}.contact-actions{justify-content:flex-start}
    .hero-editorial{display:grid;grid-template-columns:1fr 1.1fr;min-height:760px;border-radius:0;background:#fff}.hero-editorial .hero-copy{padding:80px}.hero-editorial .hero-visual{order:-1;min-height:760px}.services-editorial{background:var(--secondary)}.service-ledger{gap:14px;border:0}.service-ledger .service-item{border:0;border-radius:34px 34px 8px 34px;background:#fff;min-height:180px}.service-ledger .service-item:nth-child(2),.service-ledger .service-item:nth-child(5){background:var(--surface)}.signature-board{background:var(--primary)}.process-editorial{background:#fff}.process-editorial .process-step{border-top-color:var(--accent)}.trust-editorial{background:var(--secondary)}
